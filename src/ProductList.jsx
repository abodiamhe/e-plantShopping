...
function ProductList({ onHomeClick }) {
  ...
  //updating the total cart items in the icon
  const totalCart = () => {
    let total = 0;
    cart.forEach((cart) => {
      total += cart.quantity;
  ...
  //use to remove the 'Added to cart' css to default
  useEffect(() => {
    const updatedState = {};

    plantsArray.forEach((names) => {
      names.plants.forEach((name) => {
        //Check the cart array, if element present in cart is avalaible in the selected state
        updatedState[name.name] = cart.some((item) => item.name === name.name);
      });
  ...
  return (
    <div>
      ...
      <div className="navbar" style={styleObj}>
        ...
        {/* Plant button and cart icon in header */}
